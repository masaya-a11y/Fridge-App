...
CATEGORY_LABELS: dict[str, str] = {
    "meat": "肉類", "seafood": "魚介類", "vegetable": "野菜", "fruit": "果物",
    "dairy": "乳製品・卵", "grain": "穀物・乾物", "condiment": "調味料",
    "beverage": "飲料", "frozen": "冷凍食品", "other": "その他",
}

CATEGORY_EMOJIS: dict[str, str] = {
    "meat": "🥩", "seafood": "🐟", "vegetable": "🥦", "fruit": "🍎",
    "dairy": "🥛", "grain": "🌾", "condiment": "🧂", "beverage": "🧃",
    "frozen": "🧊", "other": "📦",
}

UNIT_OPTIONS = ["個", "本", "枚", "束", "袋", "玉", "丁", "缶",
                "g", "kg", "ml", "L", "杯", "カップ", "大さじ", "小さじ"]


def get_default_expiration(name: str, category: str) -> dict:
    info = None
    for key, val in FOOD_DATABASE.items():
        if key in name or name in key:
            ...
            break
    if info is None:
        info = CATEGORY_DEFAULTS.get(category, CATEGORY_DEFAULTS["other"])
    return {
        "date": (date.today() + timedelta(days=info["days"])).isoformat(),
...
def get_default_unit(category: str) -> str:
    return CATEGORY_DEFAULTS.get(category, CATEGORY_DEFAULTS["other"])["unit"]
